...
public class RocketLauncherItem extends Item {
    ...
    private static final float COOLDOWN_REDUCTION_PER_LEVEL = 0.2f;

    private static final SoundEvent SHOOT_SOUND = SoundEvent
            .of(new Identifier("nmsrocketlaunchermod", "rocket_launch"));
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        adjustedCooldown = Math.max(5, adjustedCooldown);
        ...
        if (world.isClient()) {
            ...
            rocket.setPosition(user.getX(), user.getEyeY() - 0.1, user.getZ());

            world.playSound(null, user.getX(), user.getY(), user.getZ(), SHOOT_SOUND,
            ...
            spawnShootParticles((ServerWorld) world, user);
        }
    ...
    private void spawnShootParticles(ServerWorld world, PlayerEntity user) {
        ...
        world.spawnParticles(ParticleTypes.CLOUD, particlePos.x, particlePos.y, particlePos.z,
                12, 0.2, 0.2, 0.2, 0.1);
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(Text.translatable("tooltip.nmsrocketlaunchermod.cooldown",
                String.format("%.1f", seconds)).formatted(Formatting.GRAY));
